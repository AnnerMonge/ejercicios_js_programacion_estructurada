...
console.log(
  `Resta: ${resta}, Multiplicación: ${multiplicacion}, División: ${division}`
);
...
console.log(
  `El precio final después de un descuento del ${descuento}% es: ${precioFinal}`
);
...
console.log(
  `¿El número ${numeroParaVerificar} es par? ${esPar(numeroParaVerificar)}`
);
...
console.log(
  `El área del rectángulo de base ${baseRectangulo} y altura ${alturaRectangulo} es: ${area}`
);
...
for (let i = 0; i < palabras.length; i++) {
  console.log(
    `La palabra "${palabras[i]}" tiene ${palabras[i].length} letras.`
  );
}
